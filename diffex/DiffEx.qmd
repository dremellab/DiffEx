...
params:
  ...
  # outdir: "~/Desktop/250424/results_250620"
...
## no ERCCs
  ...
  group <- factor(samples$groupName, levels = c(g2, g1))  # g2 is reference level
  dge <- DGEList(counts = counts, group = group)
  keep <- filterByExpr(dge, group = group)
  dge <- dge[keep,, keep.lib.sizes = FALSE]
  dge <- calcNormFactors(dge)
  
  limma_design <- model.matrix(~ 0 + group, data = samples)
  colnames(limma_design) <- levels(samples$groupName)
  # voom_plot <- NULL
  v <- voom(dge, limma_design, plot = TRUE)  # suppress direct plot
  ...
  # voom_plot <- plot(v$voom.xy$x, v$voom.xy$y, pch = 20, main = "Voom: Mean-Variance Trend",
                    # xlab = "log2(CPM)", ylab = "√(standard deviation)")
  fit <- lmFit(v, limma_design)
  contrast_str <- paste0(params$group1, " - ", params$group2)
  contrast_matrix <- makeContrasts(contrasts = contrast_str, levels = limma_design)
  fit2 <- contrasts.fit(fit, contrast_matrix)
  fit2 <- eBayes(fit2)
  top_genes <- topTable(fit2, number = Inf)
}
...
dge_edger <- DGEList(counts = ready_for_deg_raw_counts, samples = samples, group = samples$groupName)
...
# Estimate dispersion
dge_edger <- estimateDisp(dge_final, design)

# Fit GLM and run LRT
fit <- glmFit(dge_edger, design)
lrt <- glmLRT(fit)
...
as.data.frame(counts_data) %>% rownames_to_column(var = "gene") -> counts_data
gene_annotations <- counts_data %>%
  dplyr::select(gene, species, gene_chr, gene_start, gene_end,
         gene_strand, gene_length_kb, gene_type)
