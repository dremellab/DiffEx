...
params:
  ...
  # rpkm_to_counts_divisor: scaling factor used when converting ERCC-corrected log2(RPKM)
  # values back to integer counts for edgeR/DESeq2. Correct RPKM reversal uses 1.0.
  # (Historical pipeline versions hardcoded this to 20000; kept as a parameter for
  # traceability/backward comparison, not as a recommended alternative.)
  rpkm_to_counts_divisor: 1.0
...
samples <- read.table(params$samplesheet, check.names = FALSE, sep = "\t", header = TRUE, fill = TRUE)
# Drop any empty header columns from trailing delimiters
empty_cols <- which(is.na(colnames(samples)) | colnames(samples) == "")
if (length(empty_cols) > 0) {
  samples <- samples[, -empty_cols, drop = FALSE]
}
...
# Sanitize IDs used by DEG so hyphens cannot break model terms / matching
sanitize_hyphenated_values <- function(values, label) {
  values_chr <- as.character(values)
  has_hyphen <- grepl("-", values_chr, fixed = TRUE)
  if (any(has_hyphen, na.rm = TRUE)) {
    unique_bad <- unique(values_chr[has_hyphen])
    preview <- paste(head(unique_bad, 10), collapse = ", ")
    if (length(unique_bad) > 10) {
      preview <- paste0(preview, ", ...")
    }
    print(glue::glue(
      "⚠️ Warning: {sum(has_hyphen, na.rm = TRUE)} {label} value(s) contained hyphens and were converted to underscores. Examples: {preview}"
    ))
  }
  gsub("-", "_", values_chr, fixed = TRUE)
}

assert_no_hyphens <- function(values, label) {
  values_chr <- as.character(values)
  bad <- unique(values_chr[grepl("-", values_chr, fixed = TRUE)])
  if (length(bad) > 0) {
    bad_preview <- paste(head(bad, 10), collapse = ", ")
    if (length(bad) > 10) {
      bad_preview <- paste0(bad_preview, ", ...")
    }
    stop(glue::glue(
      "❌ Found hyphens in {label} after sanitization: {bad_preview}. Replace '-' with '_' before running DEG."
    ))
  }
}

samples[[params$sample_column]] <- sanitize_hyphenated_values(samples[[params$sample_column]], "sampleName")
samples[[params$group_column]]  <- sanitize_hyphenated_values(samples[[params$group_column]], "groupName")
...
counts_data <- read.table(params$counts_file,
                     ...
                     sep = "\t", 
                     header = TRUE)
counts_colnames_before <- colnames(counts_data)
counts_colnames_after <- gsub("-", "_", counts_colnames_before, fixed = TRUE)
changed_sample_cols <- sum(counts_colnames_before != counts_colnames_after)
if (changed_sample_cols > 0) {
  print(glue::glue(
    "⚠️ Warning: {changed_sample_cols} counts matrix column name(s) were converted from hyphen to underscore."
  ))
}
colnames(counts_data) <- counts_colnames_after

assert_no_hyphens(samples[[params$sample_column]], "samples.tsv sampleName")
assert_no_hyphens(samples[[params$group_column]], "samples.tsv groupName")
assert_no_hyphens(colnames(counts_data), "counts matrix column names")

counts_metadata <- counts_data[, c("species", grep("^gene_", colnames(counts_data), value = TRUE))]

gene_id_annotations <- counts_metadata %>%
  as.data.frame() %>%
  tibble::rownames_to_column(var = "gene") %>%
  dplyr::select(gene, species, gene_type)
...
orig_g1 <- params$group1
orig_g2 <- params$group2

# sanitize the group names so they match the cleaned sample sheet
# (sample values have hyphens converted to underscores earlier)
g1 <- gsub("-", "_", orig_g1)
g2 <- gsub("-", "_", orig_g2)

# Define color palette based on *sanitized* group names
group_colors <- setNames(c("#0072B2", "#D55E00"), c(g1, g2))

print(glue::glue("📥 Selecting samples in groups: "))
print(glue::glue(orig_g1, " and "))
print(glue::glue(orig_g2))
if (g1 != orig_g1 || g2 != orig_g2) {
  print(glue::glue("⚠️ Hyphens in group names replaced with underscores internally: ",
                   orig_g1, "→", g1, "; ", orig_g2, "→", g2))
}

# Filter the samplesheet to only include samples from the (sanitized) groups
selected_samples <- samples[samples[[params$group_column]] %in% c(g1, g2), ]

# Get sample IDs using the configured sample column
sample_ids <- selected_samples[[params$sample_column]]
...
# Find missing groups (use sanitized values g1/g2)
missing_groups <- setdiff(c(g1, g2), valid_groups)

if (length(missing_groups) > 0) {
  # Format available groups and missing ones with quotes for clarity
  allowed <- paste(shQuote(valid_groups), collapse = ", ")
  missing <- paste(shQuote(missing_groups), collapse = ", ")
  stop(paste0(
    "❌ Group(s) from params not found in samples[[params$group_column]]\n",
    "Allowed groups (after sanitization): ", allowed, "\n",
    "Missing group(s): ", missing, "\n",
    "⚠️ Note: hyphens in group names are converted to underscores for matching."
  ))
} else {
...
ercc_corrected_log2rpkm_counts <- log2_rpkm_counts

for (s in colnames(ercc_corrected_log2rpkm_counts)) {
  ...
  ercc_corrected_log2rpkm_counts[, s] <- (ercc_corrected_log2rpkm_counts[, s] - intercept) / slope
}

  
  ercc_corrected_log2rpkm_counts[ercc_corrected_log2rpkm_counts == -Inf] <- NA
}
```

```{r ercc_methodology}
#| code-fold: true
#| code-summary: "🧬 ERCC Normalization Methodology & Shared Helper"
#| message: false
#| warning: false
#| echo: false

# ════════════════════════════════════════════════════════════════════════════
# ERCC-based normalization: external calibration applied (Approach A)
# ════════════════════════════════════════════════════════════════════════════
#
# METHODOLOGY:
# For each sample, we fit: log2(ERCC_RPKM) ~ log2(ERCC_known_concentration)
# This gives sample-specific slope (response efficiency) and intercept (baseline).
#   - Slope ~0.85-1.0: how faithfully sequencing captures ERCC concentration changes.
#     This is a RESPONSE/SENSITIVITY factor, NOT a library-size factor.
#   - Intercept ~-15 to -18: sample-level baseline offset.
#
# Correction applied to ALL genes (not just ERCC):
#   corrected_log2_RPKM = (raw_log2_RPKM - intercept) / slope
# This inverts the calibration curve: "given this signal, what ERCC-scale
# concentration would produce it?"
#
# WHY THIS APPROACH:
# This anchors expression values to an external ERCC standard, preventing
# DESeq2/edgeR's internal normalization (median-of-ratios / TMM) from erasing
# genuine global transcriptional changes. Both of those methods assume the total
# RNA pool is roughly constant across samples -- an assumption that is violated
# in experiments such as:
#   - Pol II / CDK9 inhibition (e.g. DRB): genuine decrease in total cellular mRNA
#   - Transcription factor knockdown: widespread expression changes
#   - Host+virus sequencing: viral reads can dominate composition over time
# ERCC spike-ins are added before lysis at known concentrations and are
# unaffected by treatment, so they provide an unbiased external reference.
#
# METHOD IMPLEMENTATION (when useERCC = TRUE):
#   - limma:  uses ercc_corrected_log2rpkm_counts directly (already log2 scale,
#             so voom is never invoked on this path -- see limma section).
#   - edgeR:  converts corrected log2(RPKM) back to integer counts (below),
#             then sets norm.factors = 1 to disable TMM.
#   - DESeq2: converts corrected log2(RPKM) back to integer counts (below),
#             then sets sizeFactors = 1 to disable median-of-ratios.
#
# NOTE: This was explicitly chosen over deriving edgeR/DESeq2 size factors from
# ERCC (e.g. from ERCC library size or from the slope). The slope is a
# sensitivity factor, not a size factor, and Sarah confirmed the goal is for
# edgeR/DESeq2 to receive the same ERCC-corrected counts limma already uses,
# with their own internal normalization disabled -- not to re-derive size
# factors from ERCC and let each tool normalize as usual.
#
# If useERCC = FALSE, all three methods fall back to their standard
# normalization (voom/TMM for edgeR, median-of-ratios for DESeq2).
# ════════════════════════════════════════════════════════════════════════════

# Reverses the RPKM formula to recover integer counts from ERCC-corrected
# log2(RPKM) values, for use by edgeR and DESeq2 (which require count input).
#   RPKM = counts / (gene_length_kb * library_size_millions)
#   =>  counts = RPKM * gene_length_kb * library_size_millions
#
# `divisor` corresponds to params$rpkm_to_counts_divisor (default 1.0; the
# historical pipeline hardcoded this to 20000 with no documented rationale).
convert_ercc_corrected_log2rpkm_to_counts <- function(log2rpkm_matrix,
                                                       gene_lengths,
                                                       library_sizes,
                                                       divisor = 1.0) {
  corrected_rpkm <- 2^log2rpkm_matrix / divisor
  library_sizes_millions <- library_sizes / 1e6
  corrected_counts <- sweep(corrected_rpkm, 1, gene_lengths, "*")
  corrected_counts <- sweep(corrected_counts, 2, library_sizes_millions, "*")
...
```{r ercc_correction_comparison}
#| code-fold: true
#| code-summary: "📊 ERCC Correction: Before vs After"
#| fig.width: 10
#| fig.height: 5
#| out.width: "100%"
#| fig.align: "center"
...
if (isTRUE(params$useERCC)) {
  ...
  # Illustrate the effect of the slope/intercept correction using one
  # representative sample (the first sample in the design).
  sample_idx <- 1
  sample_name <- colnames(log2_rpkm_counts)[sample_idx]

  before_correction <- log2_rpkm_counts[, sample_idx]
  after_correction <- ercc_corrected_log2rpkm_counts[, sample_idx]

  correction_comparison <- data.frame(
    gene = rownames(log2_rpkm_counts),
    before = before_correction,
    after = after_correction[rownames(log2_rpkm_counts)]
  )
  correction_comparison <- correction_comparison[
    !apply(correction_comparison[, c("before", "after")], 1,
           function(x) any(is.na(x) | is.infinite(x))),
  ]
  correction_comparison$type <- ifelse(
    grepl("^ERCC", correction_comparison$gene), "ERCC", "Endogenous"
  )
  correction_comparison$correction_applied <-
    correction_comparison$after - correction_comparison$before

  gene_type_colors <- c("ERCC" = "#D55E00", "Endogenous" = "#0072B2")

  p1 <- ggplot(correction_comparison, aes(x = before, y = after, color = type)) +
    geom_point(alpha = 0.5, size = 1.5) +
    geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "gray40") +
    scale_color_manual(values = gene_type_colors) +
    theme_bw() +
    labs(
      title = paste0("ERCC Correction (", sample_name, ")"),
      x = "log2(RPKM) - Before",
      y = "log2(RPKM) - After ERCC Correction",
      color = "Gene Type"
    )

  p2 <- ggplot(correction_comparison, aes(x = correction_applied, fill = type)) +
    geom_histogram(alpha = 0.6, bins = 30, position = "identity") +
    scale_fill_manual(values = gene_type_colors) +
    theme_bw() +
    labs(
      title = "Distribution of Correction Magnitude",
      x = "Correction Applied (After - Before, log2 units)",
      y = "No. of genes",
      fill = "Gene Type"
    )
  ...
  ercc_mean_correction <- mean(correction_comparison$correction_applied[correction_comparison$type == "ERCC"])
  endo_mean_correction <- mean(correction_comparison$correction_applied[correction_comparison$type == "Endogenous"])
  print(glue::glue(
    "Sample {sample_name}: mean correction (log2 units) -- ",
    "ERCC = {format(ercc_mean_correction, digits = 3)}, ",
    "Endogenous = {format(endo_mean_correction, digits = 3)}"
  ))
}
```
...
add_output_annotations <- function(df, gene_id_annotations) {
  df %>%
    dplyr::left_join(gene_id_annotations, by = "gene") %>%
    tidyr::separate(gene, into = c("gene_id", "gene_name"),
...
if (isTRUE(params$useERCC)) {
  ...
  dims <- dim(ercc_corrected_log2rpkm_counts)
  
  # get batch corrected counts
  if (isTRUE(params$usebatch)){
    ercc_corrected_log2rpkm_counts_batch_corrected <- limma::removeBatchEffect(
      ercc_corrected_log2rpkm_counts,
      batch = samples[[params$batch_column]],
      design = model.matrix(~ 0 + samples[[params$group_column]])
    )
    ercc_corrected_log2rpkm_counts_batch_corrected <- as.data.frame(ercc_corrected_log2rpkm_counts_batch_corrected)
  }
  
  # Use ercc_corrected_log2rpkm_counts for limma analysis
  
  # fit the model
  fit <- lmFit(ercc_corrected_log2rpkm_counts, limma_design)
...
if (isTRUE(params$useERCC)) {
  normalized_counts_file <- file.path(counts_dir, "limma_ERCC_corrected_log2rpkm_counts.tsv")
  
    # Save normalized counts to a file
  as.data.frame(ercc_corrected_log2rpkm_counts) %>%
    rownames_to_column(var = "gene") -> ercc_corrected_log2rpkm_counts_for_output
  write.table(ercc_corrected_log2rpkm_counts_for_output,
  ...
  # Reconstruct integer-scale counts from the ERCC-corrected log2(RPKM) values.
  # This is the shared source of "ERCC-corrected counts" reused by edgeR and
  # DESeq2 below (see convert_ercc_corrected_log2rpkm_to_counts()).
  ercc_corrected_counts <- convert_ercc_corrected_log2rpkm_to_counts(
    log2rpkm_matrix = ercc_corrected_log2rpkm_counts,
    gene_lengths = merged_counts[rownames(ercc_corrected_log2rpkm_counts), "gene_length_kb"],
    library_sizes = library_sizes[colnames(ercc_corrected_log2rpkm_counts)],
    divisor = params$rpkm_to_counts_divisor
  )
  ercc_corrected_counts_for_output <- as.data.frame(ercc_corrected_counts) %>%
    rownames_to_column(var = "gene")
  ercc_corrected_counts_file <- file.path(counts_dir, "ERCC_corrected_counts.tsv")
  write.table(ercc_corrected_counts_for_output,
              file = ercc_corrected_counts_file,
              sep = "\t",
              quote = FALSE,
              col.names = TRUE,
              row.names = FALSE)
  print(glue::glue("💾 ERCC-corrected counts (shared by edgeR/DESeq2) saved to: ", ercc_corrected_counts_file))
  if (isTRUE(params$usebatch)) {
    ercc_corrected_log2rpkm_counts_batch_corrected_file <- file.path(counts_dir, "limma_ERCC_corrected_log2rpkm_counts_batch_corrected.tsv")
    as.data.frame(ercc_corrected_log2rpkm_counts_batch_corrected) %>%
      rownames_to_column(var = "gene") -> ercc_corrected_log2rpkm_counts_batch_corrected_for_output
    write.table(ercc_corrected_log2rpkm_counts_batch_corrected_for_output,
                file = ercc_corrected_log2rpkm_counts_batch_corrected_file,
                sep = "\t",
                quote = FALSE,
                col.names = TRUE,
                row.names = FALSE)
    print(glue::glue("💾 Batch corrected normalized counts saved to: ", ercc_corrected_log2rpkm_counts_batch_corrected_file))
  }
} else {
...
if (isTRUE(params$useERCC)) {
  if (isTRUE(params$usebatch)) {
    ercc_corrected_log2rpkm_counts_batch_corrected_for_plots <- ercc_corrected_log2rpkm_counts_batch_corrected[!apply(ercc_corrected_log2rpkm_counts_batch_corrected, 1, function(x) any(is.na(x) | is.infinite(x))), ]
    as.data.frame(ercc_corrected_log2rpkm_counts_batch_corrected_for_plots) %>% rownames_to_column(var = "gene") -> ercc_corrected_log2rpkm_counts_batch_corrected_for_plots
    ylabel <- "Log2 Normalized Pseudo RPKM Counts (Batch Corrected)"
    violin_plot <- make_violin_plot(ercc_corrected_log2rpkm_counts_batch_corrected_for_plots,
                                     xlab = "Sample",
                                     ylab = ylabel,
                                     title = "")
  } else {
    ercc_corrected_log2rpkm_counts_for_plots <- ercc_corrected_log2rpkm_counts[!apply(ercc_corrected_log2rpkm_counts, 1, function(x) any(is.na(x) | is.infinite(x))), ]
    as.data.frame(ercc_corrected_log2rpkm_counts_for_plots) %>% rownames_to_column(var = "gene") -> ercc_corrected_log2rpkm_counts_for_plots
    ylabel <- "Log2 Normalized Pseudo RPKM Counts"
    violin_plot <- make_violin_plot(ercc_corrected_log2rpkm_counts_for_plots,
                                     xlab = "Sample",
                                     ylab = ylabel,
                                     title = "")
  }
...
if (isTRUE(params$useERCC)) {
    if (isTRUE(params$usebatch)) {
    pcas_after <- make_pca_plots(ercc_corrected_log2rpkm_counts_batch_corrected_for_plots,
                               title = "PCA Plot of Log2 Normalized Pseudo RPKM Counts (Batch corrected)")
  } else {
    pcas_after <- make_pca_plots(ercc_corrected_log2rpkm_counts_for_plots,
...
top_genes$AveExpr <- rowMeans(ercc_corrected_log2rpkm_counts[rownames(top_genes), , drop = FALSE])
...
res_table_limma_out <- add_output_annotations(res_table_limma, gene_id_annotations) %>%
  dplyr::select(gene_id, gene_name, species, gene_type, log2FoldChange, neg_log10_pvalue,
                FoldChange, AveExpr, p_value, FDR, adjusted_pvalue, Direction, gsea_rank_score)
write.table(res_table_limma_out,
            file = limma_deg_file,
            sep = "\t",
            quote = FALSE,
            col.names = TRUE,
...
if (isTRUE(params$useERCC)) {
  # Use ERCC-corrected counts (see ercc_methodology chunk) in place of raw
  # counts, so edgeR's dispersion/filtering operate on the ERCC-calibrated scale.
  ready_for_deg_raw_counts <- round(ercc_corrected_counts)
  print(glue::glue("✅ Using ERCC-corrected counts as input to edgeR (divisor = {params$rpkm_to_counts_divisor})."))
}
...
if (isTRUE(params$useERCC)) {
  # ERCC-corrected counts already carry the external calibration -- disabling
  # TMM here (norm.factors = 1) prevents edgeR from re-normalizing against
  # endogenous genes and erasing genuine global transcriptional changes.
  # See ercc_methodology chunk above for the full rationale.
  print(glue::glue("✅ ERCC-corrected counts: disabling edgeR's TMM normalization (norm.factors = 1)."))
...
#| warning: false
# Save normalized counts to a file
# NOTE: when useERCC=TRUE, dge_final$samples$norm.factors is fixed at 1 (see
# setup_edger chunk), so this logCPM already reflects ERCC-corrected counts
# rather than TMM-normalized library sizes.
...
if (isTRUE(params$useERCC)) {
  normalized_counts_file <- file.path(counts_dir, "edgeR_ERCC_corrected_logCPM_counts.tsv")
  normalized_counts_note <- "logCPM of ERCC-corrected counts (norm.factors = 1; TMM disabled)"
} else {
  normalized_counts_file <- file.path(counts_dir, "edgeR_TMM_normalized_logCPM_counts.tsv")
  normalized_counts_note <- "TMM-normalized logCPM counts"
}
write.table(normalized_counts_edger_for_plots,
            ...
            row.names = FALSE)
print(glue::glue("💾 Normalized counts saved to: ", normalized_counts_file))
print(glue::glue("   Type: {normalized_counts_note}"))
...
if (isTRUE(params$usebatch)) {
  ...
  normalized_counts_edger_batch_corrected %>%
    rownames_to_column(var = "gene") -> normalized_counts_edger_for_plots
  if (isTRUE(params$useERCC)) {
    normalized_counts_file_batch_corrected <- file.path(counts_dir, "edgeR_ERCC_corrected_logCPM_counts_batch_corrected.tsv")
  } else {
...
res_table_edger_out <- add_output_annotations(res_table_edger, gene_id_annotations) %>%
  dplyr::select(gene_id, gene_name, species, gene_type, log2FoldChange, neg_log10_pvalue,
                FoldChange, logCPM, p_value, FDR, adjusted_pvalue, Direction, gsea_rank_score)
write.table(res_table_edger_out,
            file = results_file,
            sep = "\t",
            quote = FALSE,
            col.names = TRUE,
...
if (isTRUE(params$useERCC)) {
  # Use ERCC-corrected counts (see ercc_methodology chunk) in place of raw
  # counts, so DESeq2's dispersion/filtering operate on the ERCC-calibrated scale.
  ready_for_deg_raw_counts <- round(ercc_corrected_counts)
  print(glue::glue("✅ Using ERCC-corrected counts as input to DESeq2 (divisor = {params$rpkm_to_counts_divisor})."))
}
...
if (isTRUE(params$useERCC)) {
  # ERCC-corrected counts already carry the external calibration -- disabling
  # DESeq2's size factor estimation here (sizeFactors = 1) prevents DESeq2 from
  # re-normalizing against endogenous genes (median-of-ratios) and erasing
  # genuine global transcriptional changes. See ercc_methodology chunk above
  # for the full rationale.
  print(glue("✅ ERCC-corrected counts: disabling DESeq2's size factor estimation (sizeFactors = 1)."))
...
# Run variance-stabilizing transformation and extract assay
# use the more robust `varianceStabilizingTransformation()` which works
# for small or low-count datasets where `vst()` may fail
# NOTE: when useERCC=TRUE, sizeFactors(dds) is fixed at 1 (see
# deseq2_normalization chunk), so this VST already reflects ERCC-corrected
# counts rather than median-of-ratios normalized counts.
...
if (isTRUE(params$useERCC)) {
  normalized_counts_deseq2_file <- file.path(counts_dir, "DESeq2_ERCC_corrected_VST_normalized_counts.tsv")
  normalized_counts_deseq2_note <- "VST of ERCC-corrected counts (sizeFactors = 1; median-of-ratios disabled)"
} else {
  normalized_counts_deseq2_file <- file.path(counts_dir, "DESeq2_vst_normalized_counts.tsv")
  normalized_counts_deseq2_note <- "VST of standard median-of-ratios normalized counts"
}
...
print(glue::glue("   Type: {normalized_counts_deseq2_note}"))
...
if (isTRUE(params$usebatch)){
  ...
  # Define output file path
  if (isTRUE(params$useERCC)) {
    normalized_counts_deseq2_file_batch_corrected <- file.path(counts_dir, "DESeq2_ERCC_corrected_VST_normalized_counts_batch_corrected.tsv")
  } else {
...
res_table_deseq2_out <- add_output_annotations(res_table_deseq2, gene_id_annotations) %>%
  dplyr::select(gene_id, gene_name, species, gene_type, log2FoldChange, neg_log10_pvalue,
                FoldChange, baseMean, p_value, FDR, adjusted_pvalue, Direction, gsea_rank_score)
write.table(res_table_deseq2_out,
            file = res_file,
            sep = "\t",
            quote = FALSE,
            col.names = TRUE,
...
## ERCC Normalization Summary

```{r ercc_summary}
#| code-fold: true
#| code-summary: "🧬 ERCC Normalization Summary (All Methods)"
#| message: true
#| warning: false
#| echo: false
...
if (isTRUE(params$useERCC)) {
  print(glue::glue("
════════════════════════════════════════════════════════════════════════════
✅ ERCC-BASED NORMALIZATION SUMMARY (All Three Methods)
════════════════════════════════════════════════════════════════════════════

COMMON SOURCE: limma, edgeR, and DESeq2 all used ERCC-corrected expression
values as their starting point (see ercc_methodology chunk near the top of
this report for full rationale).

ERCC CALIBRATION (per-sample linear fit: log2(ERCC_RPKM) ~ log2(known ERCC concentration)):
  - Slope (response/sensitivity factor):  mean = {format(mean(ercc_qc$Slope), digits = 3)}, sd = {format(sd(ercc_qc$Slope), digits = 3)}
  - Intercept (sample baseline offset):    mean = {format(mean(ercc_qc$Intercept), digits = 3)}, sd = {format(sd(ercc_qc$Intercept), digits = 3)}
  - Correction: corrected_log2_RPKM = (raw_log2_RPKM - intercept) / slope
  - rpkm_to_counts_divisor used for edgeR/DESeq2 count reconstruction: {params$rpkm_to_counts_divisor}

METHOD-SPECIFIC IMPLEMENTATION:
  1. limma:  input = ERCC-corrected log2(RPKM); no voom; standard lmFit/eBayes.
  2. edgeR:  input = ERCC-corrected integer counts; norm.factors = 1 (TMM disabled).
  3. DESeq2: input = ERCC-corrected integer counts; sizeFactors = 1 (median-of-ratios disabled).

WHY: Default normalization (TMM, median-of-ratios) assumes a roughly constant
total RNA pool across samples. That assumption breaks down under genuine
global transcriptional shifts (e.g. Pol II/CDK9 inhibition, host+virus
composition shifts), where default normalization would otherwise re-normalize
away real biological signal. ERCC spike-ins are added before lysis at known
concentrations and are unaffected by treatment, providing an external,
treatment-independent scale that all three methods now share.
════════════════════════════════════════════════════════════════════════════
  "))
}
```
...
# Convert to UpSetR input and guard against empty inputs
venn_input <- UpSetR::fromList(venn_sets)

# If there are no genes / no sets to plot, skip plotting and show a message
if (is.null(venn_input) || ncol(venn_input) == 0 || nrow(venn_input) == 0) {
  message("UpSetR: no DEG sets available to plot (empty or missing). Skipping UpSet plot.")
} else {
  # Ensure nsets does not exceed available columns
  available_sets <- ncol(venn_input)
  sets_to_plot <- min(6, available_sets)
  tryCatch({
    UpSetR::upset(venn_input, nsets = sets_to_plot, order.by = "freq", main.bar.color = "#377eb8", sets.bar.color = "#e41a1c")
  }, error = function(e) {
    message("UpSetR plotting failed: ", conditionMessage(e))
  })
}
```
