...
run_gsea <- function(ranks, 
  ...
  if (!is.null(subcollection)) {
    m_df <- msigdbr(species = species, category = collection, subcategory = subcollection)
  } else {
    m_df <- msigdbr(species = species, category = collection)
  }
...
if (length(tsv_files) == 0) {
  message("No TSV files found in outdir; nothing to bundle.")
} else {
  wb <- createWorkbook()
  ...
  message("📦 Excel workbook created: ", normalizePath(xlsx_path))

  # If workbook exists, delete the per-collection TSVs
  if (file.exists(xlsx_path)) {
    unlink(tsv_files)
    message("🧹 Deleted original TSV files.")
  } else {
