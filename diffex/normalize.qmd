...
# Function to normalize values ... change all hyphens to underscore and add X prefix if starts with digit
normalize_values <- function(x) {
  x <- trimws(x)  # Trim whitespace
  x <- gsub("-", "_", x)      # Replace hyphen with underscore
  x <- gsub("^([0-9])", "X\\1", x)  # Prefix with X if starts with a digit
  return(x)
}
...
# Normalize column names in counts matrix and sample names in samplesheet
colnames(counts_data) <- normalize_values(colnames(counts_data))
samples[keep_cols] <- lapply(samples[keep_cols], normalize_values)
sample_columns <- samples[[params$sample_column]]

# Extract metadata columns from counts matrix if present
counts_metadata <- counts_data[, c( "species", grep("^gene_", colnames(counts_data), value = TRUE))]

# Ensure all sample columns are present in counts matrix
# counts = only counts
# counts_data = counts + metadata columns
# original_counts = original counts matrix before filtering
...
# this is the old way when species column was not available in the counts matrix
if (params$genes_selection == "host") {
  ...
  if (!isTRUE(params$useERCC)) {
    ...
    # Keep both host and virus genes .. keep ERCC
...
for (s in colnames(pseudo_log2_normalized_counts)) {
  ...
  # Persist ERCC-corrected log2-scale counts (pre-normalization, all genes)
  ercc_corrected_log2_counts_file <- file.path(counts_dir, "ERCC_corrected_log2_counts.tsv")

  as.data.frame(pseudo_log2_normalized_counts) %>%
    rownames_to_column(var = "gene") -> ercc_corrected_log2_counts_for_output

  # Add metadata columns
  if (exists("counts_metadata")) {
    ercc_corrected_log2_counts_for_output <- merge(
      counts_metadata, ercc_corrected_log2_counts_for_output,
      by.x = "row.names", by.y = "gene", all.y = TRUE) %>% dplyr::rename(gene_id = Row.names)
  }

  write.table(ercc_corrected_log2_counts_for_output,
              file = ercc_corrected_log2_counts_file,
              sep = "\t",
              quote = FALSE,
              col.names = TRUE,
              row.names = FALSE)
  print(glue::glue("💾 ERCC-corrected log2 counts saved to: ", ercc_corrected_log2_counts_file))

  # Linear-scale companion: pure inverse-log2, NAs preserved (not the /20000-rescaled,
  # NA-zeroed pseudo_rpkm_counts used later to feed the limma/edgeR/DESeq2 models)
  ercc_corrected_counts_file <- file.path(counts_dir, "ERCC_corrected_counts.tsv")

  as.data.frame(2^pseudo_log2_normalized_counts) %>%
    rownames_to_column(var = "gene") -> ercc_corrected_counts_for_output

  if (exists("counts_metadata")) {
    ercc_corrected_counts_for_output <- merge(
      counts_metadata, ercc_corrected_counts_for_output,
      by.x = "row.names", by.y = "gene", all.y = TRUE) %>% dplyr::rename(gene_id = Row.names)
  }
  ...
  write.table(ercc_corrected_counts_for_output,
              ...
              col.names = TRUE,
              row.names = FALSE)
  print(glue::glue("💾 ERCC-corrected linear-scale counts saved to: ", ercc_corrected_counts_file))
}
...
if (isTRUE(params$useERCC)) {
  ...
  # Add metadata columns
  if (exists("counts_metadata")) {
  pseudo_log2_normalized_counts_for_output <- merge(
  counts_metadata, pseudo_log2_normalized_counts_for_output,
  by.x = "row.names", by.y = "gene", all.y = TRUE) %>% dplyr::rename(gene_id = Row.names)
  }
  ...
  # Add metadata columns
  if (exists("counts_metadata")) {
  pseudo_rpkm_counts_for_output <- merge(
  counts_metadata, pseudo_rpkm_counts_for_output,
  by.x = "row.names", by.y = "gene", all.y = TRUE) %>% dplyr::rename(gene_id = Row.names)
  }
  ...
  if (isTRUE(params$usebatch)) {
    ...
    # Add metadata columns
    if (exists("counts_metadata")) {
    pseudo_log2_normalized_counts_batch_corrected_for_output <- merge(
    counts_metadata, pseudo_log2_normalized_counts_batch_corrected_for_output,
    by.x = "row.names", by.y = "gene", all.y = TRUE) %>% dplyr::rename(gene_id = Row.names)
    }
  ...
  as.data.frame(log2_normalized_counts_limma) %>%
    ...
    # Add metadata columns
    if (exists("counts_metadata")) {
    log2_normalized_counts_limma_for_output <- merge(
    counts_metadata, log2_normalized_counts_limma_for_output,
    by.x = "row.names", by.y = "gene", all.y = TRUE) %>% dplyr::rename(gene_id = Row.names)
    }
  ...
  if (isTRUE(params$usebatch)){
    ...
    # Add metadata columns
    if (exists("counts_metadata")) {
    log2_normalized_counts_limma_batch_corrected_for_output <- merge(
    counts_metadata, log2_normalized_counts_limma_batch_corrected_for_output,
    by.x = "row.names", by.y = "gene", all.y = TRUE) %>% dplyr::rename(gene_id = Row.names)
    }
...
# Add metadata columns
if (exists("counts_metadata")) {
  normalized_counts_edger_for_output <- merge(
  counts_metadata, normalized_counts_edger_for_plots,
  by.x = "row.names", by.y = "gene", all.y = TRUE) %>% dplyr::rename(gene_id = Row.names)
} else {
  normalized_counts_edger_for_output <- normalized_counts_edger_for_plots
}

normalized_counts_file <- file.path(counts_dir, "edgeR_TMM_normalized_logCPM_counts.tsv")
write.table(normalized_counts_edger_for_output,
            file = normalized_counts_file,
            sep = "\t",
            quote = FALSE,
            col.names = TRUE,
...
# Add metadata columns
if (exists("counts_metadata")) {
  normalized_counts_deseq2_for_output <- merge(
  counts_metadata, normalized_counts_deseq2_for_plots,
  by.x = "row.names", by.y = "gene", all.y = TRUE) %>% dplyr::rename(gene_id = Row.names)
} else {
  normalized_counts_deseq2_for_output <- normalized_counts_deseq2_for_plots
}
